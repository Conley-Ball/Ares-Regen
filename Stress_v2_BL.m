function [stressTotal,stressTow, stressTiw, stressTs, stressTc,stressPb, stressPc, stressPs, stressPt, stressPhoopow,stressPhoopiw, stressPa,vonMises] = stress(T_iw,T_ow,E_iw,E_ow,alpha_iw,alpha_ow,nu_iw,nu_ow,t_iw,t_ow,h,w,d,r_ow,r_iw,deltaP,P_chamber,A_wall,A_cc)
%{ =====STRESS===== 
...
% Compressive loading, ribs  (ignore for von mises)
stressPc = (deltaP/w)*(h+w);
% Tensile stress, ribs (ignore for von mises)
stressPt = (deltaP/w)*h;
% Shear stress, ribs (ignore for von mises)
stressPs = (deltaP/2)*(h/t_iw);
% circumferential stress in the inner and outer walls ()
stressPhoopow = deltaP*r_ow/(t_iw+t_ow);
stressPhoopiw = deltaP*r_iw/(t_iw+t_ow);
% Axial 
...
stressTotal = stressTow + stressTiw + stressTs + stressTc + stressPb + stressPc + stressPs + stressPt + stressPhoopow + stressPhoopiw + stressPa;
...
stressTotalHoop = stressTow + stressTiw + stressPhoopow + stressPhoopiw;
