    ...
    while abs(errorP) > tol
        ...
        for i=1:length(pos)
                ...
                while abs(errorT) > 1 || abs(errorT_cb) > 5
                    ...
                    eq = @(T_w) solveFunction(T_w,w,b,t,t_r,d,d_r,h,l,T_aw,M(i),T(i),A(i),gamma(i),Cp_g(i),mu_g(i),Pr_g(i),Pc,C_star,C_star_eff,D_t,R,A_t,res,T_c(i),kT_chg,kT_rhg,kT_ci,kT_ri,kT_rb,S,h_c_l,h_c_nb,T_sat_c(i),P_sat_c,ratio,T_cb_g);
                    if i > 1
