...
function [A,D,M,P,T,w_ch,D_h,w_rib,num_ch,t_ins,t_out,step,pos,D_t,A_t,Pc,Pe,mdot,A_e,D_e,MW_g,gamma,mu_g,Cp_g,k_g,Pr_g,id_th,id_c,l_div,h_ch,psi,phi,dtheta] = geometry(Thrust,Pc,Pe,mdot,A_t,D_t,A_e,D_e,C_star,C_star_eff,C_F,C_F_eff,L_star,angle_conv,w_rib,w_ch_min,MW_g,gamma,mu_g,Cp_g,k_g,Pr_g,t_ins,t_out,T_thr,num_nodes,h_ch,pitch,max_angle,throat_only)
    ...
    x3 = 0.682 * D_t / 2 .* cosd(t3);
    y3 = 0.682 * D_t / 2 * sind(t3) + 0.682 * D_t / 2 + D_t/2;
    ...
    dr = diff(r);
    dr = [dr dr(end)];
    if throat_only == false
        alpha = pitch*pi/180;
    else
        alpha = pitch*pi/180;
        alpha = [linspace(0,pitch*pi/180,100) linspace(pitch*pi/180,0,100) zeros(1,100)];
        alpha = interp1(x,alpha,pos);
    end
    dtheta = tan(alpha)./r.*(dz.^2+dr.^2).^0.5;
    psi = atan(dr./dz);
    phi = atan((dr.^2+(r.*dtheta).^2).^0.5./dz);
    step = dz;

    if max(phi) > max_angle*pi/180
